...
import { FileText, Sparkles, Clock, ChevronRight, ChevronLeft, RefreshCw, BookOpen, Brain, Edit3, Check, X, Loader2, Crown, Zap } from 'lucide-react';
import { useAuthStore } from '../stores/authStore';
import { smartReviewService } from '../services/smartreview.service';
import FileUpload from '../components/FileUpload';
import Header from '../components/Header';
import type { UserQuota, QuizListItem, Document } from '../types/smartreview';
...
export default function SmartReviewPage() {
  ...
  // Track previous processing count to detect completion
  ...
  useEffect(() => {
    ...
    // Check if any processing just completed
    if (prevProcessingCountRef.current > 0 && processingCount === 0) {
      // Quiz just completed - show success and reload
    ...
    if (hasProcessing && token) {
      const interval = setInterval(async () => {
        try {
          ...
          const stillProcessing = docsData.filter(d => d.status === 'processing').length;
          setDocuments(docsData);
          
          // If processing just completed, reload quizzes
          if (stillProcessing === 0 && processingCount > 0) {
            const quizzesData = await smartReviewService.getQuizzes(token, 1, QUIZZES_PER_PAGE);
      ...
      }, 2000); // Poll faster - every 2 seconds
  ...
  const loadInitialData = async () => {
    ...
    try {
      setIsLoading(true);
      setPage(1);
      const [quotaData, quizzesData, docsData] = await Promise.all([
        smartReviewService.getQuota(token),
        smartReviewService.getQuizzes(token, 1, QUIZZES_PER_PAGE),
        smartReviewService.getDocuments(token),
      ]);
      setQuota(quotaData);
      setQuizzes(quizzesData.quizzes);
      setHasMore(quizzesData.hasMore);
  ...
  const loadPage = async (pageNum: number) => {
    ...
    try {
      setIsLoading(true);
      const data = await smartReviewService.getQuizzes(token, pageNum, QUIZZES_PER_PAGE);
  ...
  const handleUpload = async (files: File[]) => {
    ...
    try {
      ...
      if (files.length === 1) {
        await smartReviewService.uploadDocument(files[0], token);
      } else {
        await smartReviewService.uploadMultipleDocuments(files, token);
      }
  ...
  const saveTitle = async (quizId: string) => {
    ...
    try {
      ...
      // Update local state
      setQuizzes(prev =>
        prev.map(q => q._id === quizId ? { ...q, title: editTitle.trim() } : q)
      );
      setEditingId(null);
    } catch (err) {
      ...
      <main className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-10 text-center sm:text-left">
          ...
          <p className="text-lg text-slate-600 max-w-2xl">
            Upload tài liệu học tập và để AI tạo <span className="font-semibold text-blue-600">Quiz</span> & <span className="font-semibold text-cyan-600">Flashcard</span> giúp bạn ôn thi hiệu quả
          </p>
        </div>
        ...
        {successMessage && (
          <div className="mb-6 p-4 rounded-2xl bg-gradient-to-r from-emerald-50 to-green-50 border border-emerald-200/50 flex items-center gap-3 animate-in slide-in-from-top duration-300">
            ...
            <p className="text-emerald-700 flex-1 font-medium">{successMessage}</p>
            <button onClick={() => setSuccessMessage(null)} className="text-emerald-500 hover:text-emerald-700 font-medium">
              Đóng
            </button>
          </div>
        )}
            ...
            <div className="sticky top-24">
              <div className="bg-white/70 backdrop-blur-xl rounded-3xl p-6 border border-white/50 shadow-xl shadow-slate-200/50">
                <div className="flex items-center gap-3 mb-5">
                  ...
                  <div>
                    <h2 className="font-bold text-slate-800">Tải lên tài liệu</h2>
                    <p className="text-xs text-slate-500">PDF, Word, PowerPoint</p>
                  </div>
                </div>
                ...
                {documents.filter(d => d.status === 'processing').length > 0 && (
                  <div className="mt-5 p-4 rounded-2xl bg-gradient-to-r from-blue-50 to-cyan-50 border border-blue-200/50">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-blue-700">AI đang phân tích...</p>
                        <p className="text-sm text-blue-600">Đang tạo quiz từ tài liệu của bạn</p>
                      </div>
                    </div>
                  </div>
                )}
            ...
            <div className="bg-white/70 backdrop-blur-xl rounded-3xl p-6 border border-white/50 shadow-xl shadow-slate-200/50">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">{totalQuizzes} bộ quiz</p>
                  </div>
                </div>
                <button
                  ...
                  title="Refresh"
                >
                  <RefreshCw className={`w-5 h-5 text-slate-600 group-hover:text-slate-800 ${isLoading ? 'animate-spin' : ''}`} />
                </button>
              </div>
              ...
              {isLoading ? (
                ...
                <div className="py-16 text-center">
                  ...
                  <h3 className="text-lg font-semibold text-slate-700 mb-2">Chưa có quiz nào</h3>
                  <p className="text-slate-500">Upload tài liệu để AI tạo quiz cho bạn</p>
                </div>
              ) : (
                <div className="space-y-3">
                        ...
                        <div className="flex-1 min-w-0">
                          {editingId === quiz._id ? (
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <button
                                onClick={(e) => { e.stopPropagation(); startEditing(quiz); }}
                  ...
                  {totalQuizzes > QUIZZES_PER_PAGE && (
                    <div className="mt-6 flex items-center justify-center gap-2">
                      ...
                      <div className="flex items-center gap-1">
                        {Array.from({ length: Math.ceil(totalQuizzes / QUIZZES_PER_PAGE) }, (_, i) => i + 1)
                          .filter(p => {
                  ...
                  {totalQuizzes > 0 && (
                    <p className="text-center text-slate-400 text-sm mt-4">
                      Trang {page} / {Math.ceil(totalQuizzes / QUIZZES_PER_PAGE)} • Tổng {totalQuizzes} quiz
                    </p>
                  )}
                </div>
              )}
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white/95 backdrop-blur-xl rounded-3xl p-8 max-w-md w-full text-center shadow-2xl border border-white/50 animate-in zoom-in-95 duration-300">
            ...
            <p className="text-slate-600 mb-8">
              Bạn đã hết lượt upload miễn phí. Nâng cấp lên <span className="font-semibold text-amber-600">Premium</span> để upload không giới hạn!
            </p>
            ...
            <div className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-2xl p-4 mb-6 text-left">
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="text-slate-700">AI nâng cao với độ chính xác cao hơn</span>
                </div>
                <div className="flex items-center gap-2 text-sm">
                  <Crown className="w-4 h-4 text-amber-600" />
                  <span className="text-slate-700">Ưu tiên xử lý nhanh hơn</span>
                </div>
              </div>
            </div>
